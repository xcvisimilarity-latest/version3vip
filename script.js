...
async function authenticatedFetch(action = '', body = {}) {
  ...
  // Tentukan method dan config berdasarkan action
  ...
  // Hanya tambah body untuk non-GET
  ...
  try {
    ...
    if (result.ok && Array.isArray(result.senders)) {
      ...
      userSenders.forEach(sender => {
        ...
        // Simpan sebagai composite value
...
document.getElementById('attackForm').addEventListener('submit', (e) => {
  ...
  // Di dalam attack form handler, perbaiki bagian pencarian sender:
(async () => {
    ...
    if (all.ok && Array.isArray(all.senders)) {
      ...
      if (senderName.includes('||')) {
        ...
        const found = all.senders.find(s => 
          (s.name || '') === senderNameOnly && s.status === 'connected'
        );
