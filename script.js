...
// REPLACEMENT: fullscreen overlay-aware showToolContent
function showToolContent(tool) {
  const toolContent = document.getElementById('toolContent');
  const overlay = document.getElementById('toolOverlay');
  const overlayContent = document.getElementById('toolOverlayContent');
  ...
  switch (tool) {
   case 'banned':
    ...
    // ... copy existing cases from your original showToolContent ...
    // Keep the other cases (spamngl, checkip, stalkig, stalktt, iqc, etc.)
    // For brevity here, keep them identical to the old implementation.
    default:
      content = `<div class="card"><p style="text-align:center;">Pilih tool untuk melihat detailnya</p></div>`;
  }

  // STILL render to #toolContent (backwards compatibility)
  if (toolContent) toolContent.innerHTML = content;

  // Render to overlay and open it
  if (overlay && overlayContent) {
    overlayContent.innerHTML = `<div class="overlay-body">${content}</div>`;
    ...
    const closeOverlay = () => {
      ...
      setTimeout(() => { overlay.style.display = 'none'; overlayContent.innerHTML = ''; }, 220);
    };

    // bind close button
    const closeBtn = document.getElementById('closeToolOverlay');
    if (closeBtn) {
      closeBtn.onclick = closeOverlay;
    }

    // click backdrop to close
    const backdrop = document.getElementById('toolOverlayBackdrop');
    if (backdrop) {
      backdrop.onclick = closeOverlay;
    }

    // bind dynamic form handlers inside overlay
    const handlers = {
      ...
      iqc: handleIQC
      // add other mappings if you create more handlers
    };

    // If there is a form inside overlay, attach the proper handler
    const form = overlayContent.querySelector('form');
    if (form) {
      // remove previous same listener if any (defensive)
      const cloned = form.cloneNode(true);
      form.parentNode.replaceChild(cloned, form);
      const usedHandler = handlers[tool];
      if (usedHandler) {
        cloned.addEventListener('submit', usedHandler);
      } else {
        // generic: prevent default
        cloned.addEventListener('submit', (e) => e.preventDefault());
      }
    }

    // Ensure other buttons (like vercel deploy) still bind when rendered in overlay
    // Example: if vercel form is present, re-bind its submit
    const vercelForm = overlayContent.querySelector('#vercelDeployForm');
    if (vercelForm) {
      vercelForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        // use existing logic: find the original function body (kept elsewhere) or
        // reuse same code as before that posted to /api/connect?deploy-vercel
        ...
        try {
          const formData = new FormData(vercelForm);
          ...
          if (result.ok) {
            ...
            if (resultDiv) resultDiv.innerHTML = `<p style="color:#ff0040">Gagal: ${result.error || 'Unknown'}</p>`;
            showToast(result.error || 'Deploy gagal', 'error');
          }
        } catch (err) {
          console.error('Deploy error overlay:', err);
          showToast('Terjadi kesalahan deploy', 'error');
        } finally {
          if (btn) { btn.disabled = false; btn.innerHTML = `<i class="fas fa-cloud-upload-alt" style="margin-right:8px;"></i>Deploy Sekarang`; }
        }
      });
    }
  }
