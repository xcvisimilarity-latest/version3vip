...
document.getElementById('attackForm').addEventListener('submit', async (e) => {
  ...
  if (!attackOption) { 
    showToast('Pilih opsi serangan!', 'error'); 
    return; 
  }
  if (!senderName) { 
    showToast('Pilih sender!', 'error'); 
    return; 
  }
  ...
  let senderNameOnly = senderName;
  
  console.log('[DEBUG] Starting attack with sender:', senderName);
  
  try {
    const all = await authenticatedFetch('senders');
    console.log('[DEBUG] All senders response:', all);
    
    if (all.ok && Array.isArray(all.senders)) {
      console.log('[DEBUG] Available senders:', all.senders);
      
      // Handle composite value (name||phone)
      if (senderName.includes('||')) {
        [senderNameOnly, senderPhone] = senderName.split('||');
        console.log('[DEBUG] Parsed composite sender:', { senderNameOnly, senderPhone });
      } else {
        // Cari berdasarkan name saja
        const found = all.senders.find(s => {
          const match = (s.name || '') === senderNameOnly && s.status === 'connected';
          console.log('[DEBUG] Checking sender:', s.name, 'match:', match, 'status:', s.status);
          return match;
        });
        
        if (found) {
          senderPhone = String(found.phone || '');
          console.log('[DEBUG] Found sender by name:', found);
        }
      }

      // FALLBACK: Jika masih tidak ketemu, cari dengan logika lebih longgar
      if (!senderPhone) {
        console.log('[DEBUG] Trying fallback sender search...');
        const foundFallback = all.senders.find(s => {
          const sName = s.name || '';
          const sPhone = String(s.phone || '');
          const nameMatch = sName === senderNameOnly || 
                           sName.includes(senderNameOnly) || 
                           senderNameOnly.includes(sName);
          const statusMatch = s.status === 'connected';
          
          console.log('[DEBUG] Fallback check:', { 
            sName, senderNameOnly, nameMatch, statusMatch 
          });
        ...
        if (foundFallback) {
          senderPhone = String(foundFallback.phone || '');
          senderNameOnly = foundFallback.name || senderNameOnly;
          console.log('[DEBUG] Found sender via fallback:', foundFallback);
        }
      }
    } else {
      console.error('[DEBUG] Senders API error:', all);
    }
  } catch (err) {
    console.error('[DEBUG] Error fetching senders:', err);
    showToast('Gagal mengambil data sender.', 'error');
    return;
  }

  if (!senderPhone) {
    console.error('[DEBUG] No sender phone found for:', senderName);
    showToast('Gagal mendapatkan nomor sender. Pastikan sender terhubung dan coba lagi.', 'error');
    return;
  }

  console.log('[DEBUG] Final sender info:', { 
    senderNameOnly, 
    senderPhone, 
  ...
  // --- Optimistic UI ---
  loading.classList.add('active');
  attackStatus.textContent = 'Memulai...';
  attackDetails.textContent = `Target: ${cleanTarget} | Opsi: ${attackOption.toUpperCase()} | Sender: ${senderNameOnly} | Jumlah: ${attackCount}x`;
  ...
  attackDetails.insertAdjacentHTML('afterend', `
    <div id="${statusEntryId}" class="attack-entry" style="margin-top:10px;">
      <div class="attack-entry-header">
        ...
        <span class="entry-status">Memulai ${attackCount}x serangan</span>
      </div>
      <div class="attack-entry-body">
        <div class="progress-bar" style="width:100%; background:rgba(255,255,255,0.06); height:8px; border-radius:6px; overflow:hidden; margin-top:8px;">
          <div class="progress-fill" style="width:10%; height:100%; transition: width 400ms ease; background: var(--primary);"></div>
        </div>
  ...
  try {
    const payload = { 
      name: senderNameOnly,  // Gunakan nama saja, bukan composite
      phone: senderPhone,    // Gunakan nomor terpisah
      to: cleanTarget,
      count: attackCount
    };

    console.log('[DEBUG] Sending attack payload:', payload);
    ...
    console.log('[DEBUG] Attack response:', res);
    ...
    if (res && Array.isArray(res.progress) && res.progress.length) {
      ...
      const completed = res.progress.filter(p => 
        p.includes('Selesai') || p.includes('sukses') || p.includes('✅')
      ).length;
    ...
    if (res && res.ok) {
      ...
      attackStatus.style.color = 'var(--primary)';
      ...
      showToast(`Serangan ${attackOption} berhasil!`, 'success');
    } else {
      progressFill.style.width = '100%';
      ...
      attackStatus.style.color = 'var(--secondary)';
      ...
      showToast(`Serangan gagal: ${errMsg}`, 'error');
    }
  } catch (err) {
    console.error('[DEBUG] Attack background error:', err);
    progressFill.style.width = '100%';
    progressFill.style.background = 'var(--secondary)';
    entryStatus.textContent = 'Gagal';
    ...
    showToast('Kesalahan jaringan saat memproses serangan.', 'error');
  } finally {
    loading.classList.remove('active');
  }
});

// ====== PERBAIKAN loadSendersForAttack ======
async function loadSendersForAttack() {
  try {
    const user = JSON.parse(localStorage.getItem('user') || '{}');
    const username = user.username || userData?.username || null;

    console.log('[DEBUG] Loading senders for user:', username);
    
    const result = await authenticatedFetch('senders');
    console.log('[DEBUG] Senders API response:', result);
    
    const senderSelect = document.getElementById('senderSelect');
    if (!senderSelect) {
      console.error('[DEBUG] senderSelect element not found');
      return;
    }
    ...
    defaultOption.value = "";
    defaultOption.textContent = "Pilih sender";
    defaultOption.disabled = true;
    defaultOption.selected = true;
    senderSelect.appendChild(defaultOption);

    if (result.ok && Array.isArray(result.senders)) {
      console.log('[DEBUG] Raw senders:', result.senders);
      
      const userSenders = result.senders.filter(sender => {
        const isUserSender = (sender.owner || '').toLowerCase() === (username || '').toLowerCase();
        const isGlobalSender = sender.isGlobal === true || sender.owner === 'global';
        ...
        return (isUserSender || isGlobalSender) && isConnected;
      });

      console.log('[DEBUG] Filtered senders for attack:', userSenders);

      if (userSenders.length === 0) {
        const option = document.createElement('option');
        option.value = "";
        option.textContent = "Tidak ada sender tersedia";
        option.disabled = true;
        senderSelect.appendChild(option);
        console.warn('[DEBUG] No available senders after filtering');
      } else {
        userSenders.forEach(sender => {
          const option = document.createElement('option');
          const safeName = String(sender.name || 'unknown');
          const safePhone = String(sender.phone || 'unknown');
          
          // Simpan sebagai composite value untuk konsistensi
          option.value = `${safeName}||${safePhone}`;
          option.textContent = `${safeName} (${safePhone})${sender.isGlobal ? ' - GLOBAL' : ''}`;
          option.setAttribute('data-name', safeName);
          option.setAttribute('data-phone', safePhone);
          senderSelect.appendChild(option);
        });
        
        console.log('[DEBUG] Added', userSenders.length, 'senders to dropdown');
      }
    } else {
      console.warn('[DEBUG] No senders found or API error:', result);
      const option = document.createElement('option');
      option.value = "";
      option.textContent = "Error memuat sender - " + (result.error || 'Unknown error');
      option.disabled = true;
      senderSelect.appendChild(option);
    }
  } catch (error) {
    console.error('[DEBUG] Error loading senders for attack:', error);
    const senderSelect = document.getElementById('senderSelect');
    if (senderSelect) {
      senderSelect.innerHTML = '<option value="" disabled selected>Error memuat sender</option>';
    }
  }
}

// ====== PERBAIKAN authenticatedFetch ======
async function authenticatedFetch(action = '', body = {}) {
  ...
  // Tentukan method berdasarkan action
  const isGetRequest = ['senders', 'status'].includes(action);
  ...
  // Hanya tambah body untuk non-GET request
  if (!isGetRequest && Object.keys(body).length > 0) {
    config.body = JSON.stringify(body);
  }

  console.log(`[DEBUG] authenticatedFetch: ${action}`, { 
  ...
  try {
    ...
    try {
      ...
      console.log(`[DEBUG] authenticatedFetch response for ${action}:`, result);
      return result;
    } catch (e) {
