...
// Event listeners untuk modal tools
document.getElementById('toolModalClose').addEventListener('click', closeToolModal);
document.getElementById('toolModalOverlay').addEventListener('click', closeToolModal);

// ====== UPDATED TOOLS FUNCTIONALITY ======
function showToolContent(tool) {
  ...
  switch (tool) {
    // 🔹 CEK BANNED
    case 'banned':
      title = 'Cek Status Banned WhatsApp';
      content = `
    ...
    case 'ttdownloader':
      ...
      content = `
        <h3 style="text-align:center;margin-bottom:15px;color:var(--primary);">${title}</h3>
    ...
    default:
      title = 'Tool';
      content = `<p style="text-align:center;">Pilih tool untuk melihat detailnya</p>`;
  }

  // Tampilkan modal dengan konten tool
  showToolModal(title, content);

  // Bind event handlers setelah konten dimuat
  setTimeout(() => {
    const handlers = {
...
// Update event listener untuk tool cards
document.querySelectorAll('.tool-card').forEach(card => {
  card.addEventListener('click', function() {
    const tool = this.getAttribute('data-tool');
    showToolContent(tool);
